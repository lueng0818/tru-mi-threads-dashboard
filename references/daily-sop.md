...
- 對外帳號一律 `@trumi_jewelryofficial`（舊帳號 `@trumi_jewelry` 已停用）。
  ⚠️ **2026-09-11 修訂**：原本這裡寫「新卡片與今日草稿結尾固定『完整故事看 IG …』」，
  該條已**取消**，改依 §4.7.4 判斷要不要提 IG。帳號名稱本身維持不變。
...
5. Threads 延伸貼文草稿（設計師第一人稱；100-200 字是**篇幅參考不是門檻**，
   結尾**不強制**加 IG 導流——兩項都見 §4.7.4）
...
| 長度與格式 | 短、可不完整 | 第一人稱；100-200 字為參考值，講完就停；結尾不強制 CTA（見 4.7.4） |
...
1. **開場依原始內容與 Jessica 真正要延伸的點自然開始，不指定固定句型。**（2026-09-11 改寫）

   生成前先判斷：這一篇真正要延伸的是**原文中的哪一句話、哪個具體情境、哪個問題、
   Jessica 的哪個觀察，還是某項必要的專業資訊**？從那個內容直接進入。

   ❌「很多人以為婚戒就是⋯⋯」——從一個道理開始，那是文案的開場。

   ⚠️ 「今天看到一則貼文，她說⋯⋯」**只是某一情境下可能成立的寫法**，
   ⛔ 不得作為預設開場、推薦模板或優先句型。

   ⛔ **也不得為了避免重複而建立另一組開場模板或輪替句庫。**
   把 99 則同一句換成 5 種開場各 20 則，仍然是模板。

   > **修訂理由（2026-09-11）**：458 段正式重跑後的 Corpus Audit 顯示，
   > 146 則延伸貼文有 **99 則（67%）** 以「（今天）看到一則貼文⋯」開場，跨大量不同主題。
   > 追查原因是本節初版把這一句寫成 ✅ 範例，執行端就收斂到它。
   > 這與 §4.7.4ⓑ 撤回的固定 IG 結尾是**同一類問題：system-rule-caused template**——
   > 來源是規則不是模型漂移，所以「AI Pattern Audit」那組檢查抓不到它，
   > 要靠 Corpus Audit 才會浮現。
...
#### 4.7.4 格式（2026-09-11 修訂兩條，其餘不變）

**維持不變**：第一人稱、素材四格（4.5）、去 AI 味終檢（4.6）、禁用詞、不放連結、不放 hashtag。

##### ⓐ 100–200 字改為參考值，不是驗收門檻

```
100–200 字是篇幅參考，不是驗收門檻。
內容自然講完即可停止。
⛔ 不得為達字數補充案例、觀點、情緒、結論或 CTA。
```

> **修訂理由（2026-09-11）**：218 段第二輪重檢時，card 99／117／124 三則重寫後
> 分別是 86／98／96 字。要補到 100 字，只能加一句不是對方需要的話——
> 那會直接違反 `jessica-voice.md §2.5.4`「停止點優先於完整性」。
> 字數是低位規則，停止點是高位規則，衝突時以停止點為準。
> 當時「不到 100 字但不硬補」的處置是**正確的**，不再視為規則衝突。

##### ⓑ 取消「固定 IG 導流結尾」

```
Threads 延伸貼文不得強制固定 IG 導流結尾。
...
⛔ 不得為格式完整，讓所有延伸貼文出現相同 CTA。
```

> **修訂理由（2026-09-11）**：原規定要求每一則都以
> 「完整故事看 IG @trumi_jewelryofficial」收尾。這與三件已確立的事直接衝突：
> Threads 以聊天感為主（§4.7.2）｜不得為完整性強塞 CTA（`jessica-voice.md §2.5.4`）｜
> 海巡不放連結、不自薦（§2.3）。
> 而且它本身就是一個**跨主題完全相同的收尾**——
> 正是 `jessica-voice.md §7.4` Corpus Audit 要抓的模板，
> 只是它**由系統規定造成，不是模型自己長出來的**，所以過去的稽查一直放行它。
> 第二輪重檢時曾機械式地把它補回 58 則，該處置已撤回。
>
> ⚠️ 另記：帳號實際使用名稱是否仍與這個 handle 一致，需要另外確認，
> 不得只因 canonical 寫過固定格式就重複套用。
